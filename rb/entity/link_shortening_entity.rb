...
class LinkShorteningEntity
  ...
  def make
    opts = @_entopts.dup
    LinkShorteningEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = UToLinkShortenerHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [LinkShortening, Hash] the current LinkShortening data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = UToLinkShortenerHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of LinkShortening fields)
  def match_get
  ...
  #
  # @param reqdata [LinkShorteningCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [LinkShortening, Hash] the created LinkShortening; raises UToLinkShortenerError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = UToLinkShortenerHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
