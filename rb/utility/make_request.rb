...
module UToLinkShortenerUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = UToLinkShortenerResponse.new({})
    ...
    if fetch_err
      ...
      response = UToLinkShortenerResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = UToLinkShortenerResponse.new(fetched)
    else
