...
// UToLinkShortener SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class UToLinkShortenerMakeError
{
    public static function call(?UToLinkShortenerContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new UToLinkShortenerContext([], null);
        }
        $op = $ctx->op ?? new UToLinkShortenerOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new UToLinkShortenerResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof UToLinkShortenerError) ? $err->msg : (string)$err;
        $msg = "UToLinkShortenerSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new UToLinkShortenerError('', $msg, $ctx);
        ...
        if ($err instanceof UToLinkShortenerError) {
