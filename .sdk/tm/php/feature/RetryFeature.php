...
// UToLinkShortener SDK retry feature
...
class UToLinkShortenerRetryFeature extends UToLinkShortenerBaseFeature
{
    ...
    public function init(UToLinkShortenerContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (UToLinkShortenerContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(UToLinkShortenerContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(UToLinkShortenerContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
