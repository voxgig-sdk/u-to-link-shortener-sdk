...
module UToLinkShortenerUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = UToLinkShortenerContext.new({}, nil)
    end
    op = ctx.op || UToLinkShortenerOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || UToLinkShortenerResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(UToLinkShortenerError) ? err.msg : err.to_s
    ...
    sdk_err = UToLinkShortenerError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(UToLinkShortenerError)
