...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = UToLinkShortenerOmni.make_runner(TEST_JSON_FILE, UToLinkShortenerSDK.test(nil, nil))
  ...
  def err_from_map(m)
    ...
    UToLinkShortenerError.new(m["code"] || "", msg)
  end
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : UToLinkShortenerOmni.errify(err)
    end
  ...
  def test_operator_basic
    subject = lambda do |vin|
      vin = {} unless vin.is_a?(Hash)
      op = UToLinkShortenerOperation.new(vin)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
