...
from projectname_sdk.core.control import UToLinkShortenerControl
from projectname_sdk.core.error import UToLinkShortenerError


def make_error_util(ctx, err):
    if ctx is None:
        from projectname_sdk.core.context import UToLinkShortenerContext
        ctx = UToLinkShortenerContext({}, None)

    op = ctx.op
    if op is None:
        op = UToLinkShortenerOperation({})
    opname = op.name
    ...
    errmsg = ""
    if isinstance(err, UToLinkShortenerError):
        errmsg = err.msg
    ...
    sdk_err = UToLinkShortenerError("", msg, ctx)
