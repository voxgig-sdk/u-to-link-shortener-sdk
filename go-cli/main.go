// Command UToLinkShortener-cli is a CLI + REPL for the UToLinkShortener SDK,
...
const prompt = "u-to-link-shortener"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "link_shortening"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      U_TO_LINK_SHORTENER_APIKEY (api key), U_TO_LINK_SHORTENER_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: U_TO_LINK_SHORTENER_APIKEY carries the API key and
	// U_TO_LINK_SHORTENER_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("U_TO_LINK_SHORTENER_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("U_TO_LINK_SHORTENER_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewUToLinkShortenerSDK(opts)
