# u-to-link-shortener-cli

boru-driven command-line client **and** interactive REPL for the UToLinkShortener
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/u-to-link-shortener-cli)
make build

# 2. See usage (words, entities, env vars)
./u-to-link-shortener-cli --help
...
U_TO_LINK_SHORTENER_BASE=https://api.example.com ./u-to-link-shortener-cli --help

# 6. No arguments -> interactive REPL
./u-to-link-shortener-cli
u-to-link-shortener> /help
u-to-link-shortener> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/u-to-link-shortener-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/u-to-link-shortener-cli --help
   ```
...
Run with no arguments to open a REPL (prompt `u-to-link-shortener>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `link_shortening`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/u-to-link-shortener-cli`. |
...
(`./u-to-link-shortener-cli <expr>`) and interactively (the REPL), and expressions compose the
