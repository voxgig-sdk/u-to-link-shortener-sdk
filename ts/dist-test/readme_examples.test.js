...
const __1 = require("..");
const SDK_NAME = 'UToLinkShortenerSDK';
...
const TEST_SEED = { "entity": { "link_shortening": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+UToLinkShortenerSDK\b/.test(code) ||
        /\bUToLinkShortenerSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+UToLinkShortenerSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/UToLinkShortenerSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
