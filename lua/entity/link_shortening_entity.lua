...
local LinkShorteningEntity = {}
LinkShorteningEntity.__index = LinkShorteningEntity


function LinkShorteningEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, LinkShorteningEntity)
  self._name = "link_shortening"
  self._client = client
...
function LinkShorteningEntity:get_name()
  return self._name
end


function LinkShorteningEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return LinkShorteningEntity.new(self._client, opts)
end
...
function LinkShorteningEntity:mark_deleted()
  self._deleted = true
end


function LinkShorteningEntity:deleted()
  return true == self._deleted
end


function LinkShorteningEntity:data_set(args)
  if args ~= nil then
...
function LinkShorteningEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function LinkShorteningEntity:match_set(args)
  if args ~= nil then
...
function LinkShorteningEntity:match_get()
...
function LinkShorteningEntity:stream(action, args, callopts)
...
---@param reqdata LinkShorteningCreateData
---@param ctrl? table
---@return LinkShortening
---@return string? err
function LinkShorteningEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function LinkShorteningEntity:_run_op(ctx, post_done)
  local utility = self._utility
