...
describe("LinkShorteningEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set UTOLINKSHORTENER_TEST_LINK_SHORTENING_ENTID JSON to run live")
      return
    end
...
function link_shortening_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("UTOLINKSHORTENER_TEST_LINK_SHORTENING_ENTID")
  ...
  if env["UTOLINKSHORTENER_TEST_LIVE"] == "TRUE" then
  ...
  local live = env["UTOLINKSHORTENER_TEST_LIVE"] == "TRUE"
  return {
    client = client,
    data = entity_data,
