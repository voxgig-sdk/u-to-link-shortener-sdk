# UToLinkShortener Lua SDK Reference

Complete API reference for the UToLinkShortener Lua SDK.
...
```lua
local link_shortening = client:LinkShortening(nil)
```
...
```lua
local result, err = client:LinkShortening():create({
  url = --[[ string ]],
})
```
