...
from utolinkshortener_sdk.utility import register

# Load features
from utolinkshortener_sdk.feature.base_feature import UToLinkShortenerBaseFeature
from utolinkshortener_sdk.features import _has_feature, _make_feature
...
class UToLinkShortenerSDK:
    ...
    def get_utility(self):
        return UToLinkShortenerUtility.copy(self._utility)
    ...
    def graphql(self, query, variables=None, ctrl=None):
        ...
        if isinstance(errors, list) and 0 < len(errors):
            ...
            res["ok"] = False
            res["err"] = Exception("UToLinkShortenerSDK: graphql: " + str(msg))
            res["graphql"] = errors

        return res


    def LinkShortening(self, data=None) -> "LinkShorteningEntity":
        """Entity factory: client.LinkShortening().list() / client.LinkShortening().load({"id": ...})."""
        from utolinkshortener_sdk.entity.link_shortening_entity import LinkShorteningEntity
        return LinkShorteningEntity(self, data)



    @classmethod
    def test(cls, testopts=None, sdkopts=None) -> "UToLinkShortenerSDK":
        if sdkopts is None:
            sdkopts = {}
