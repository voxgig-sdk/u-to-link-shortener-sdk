# UToLinkShortener SDK test runner
...
class UToLinkShortenerTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            UToLinkShortenerTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = UToLinkShortenerTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = UToLinkShortenerTestRunner.getenv("U_TO_LINK_SHORTENER_TEST_LIVE")
        override = UToLinkShortenerTestRunner.getenv("U_TO_LINK_SHORTENER_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = UToLinkShortenerTestRunner.getenv(key)
        ...
        explain = UToLinkShortenerTestRunner.getenv("U_TO_LINK_SHORTENER_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if UToLinkShortenerTestRunner._test_control is not None:
            return UToLinkShortenerTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                UToLinkShortenerTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            UToLinkShortenerTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = UToLinkShortenerTestRunner.load_test_control()
...
def load_env_local():
    UToLinkShortenerTestRunner.load_env_local()


def env_override(m):
    return UToLinkShortenerTestRunner.env_override(m)


def entity_data(v):
    return UToLinkShortenerTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return UToLinkShortenerTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return UToLinkShortenerTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return UToLinkShortenerTestRunner.load_test_control()


def live_client_options():
    return UToLinkShortenerTestRunner.live_client_options()


def live_delay_ms():
    return UToLinkShortenerTestRunner.live_delay_ms()
