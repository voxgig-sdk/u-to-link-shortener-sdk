...
// UToLinkShortener SDK LinkShortening entity
...
class LinkShorteningEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new LinkShorteningEntity($this->_client, $opts);
    }

    /**
     * @param LinkShortening|array $args LinkShortening data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = UToLinkShortenerHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return LinkShortening|array The current LinkShortening data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of LinkShortening fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = UToLinkShortenerHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of LinkShortening fields).
     */
    ...
    /**
     * Create a new LinkShortening.
     *
     * @param LinkShorteningCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed LinkShorteningCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return LinkShortening|array The created LinkShortening as an assoc-array at the
     *   SDK boundary; throws UToLinkShortenerError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                if ($ctx->result->resdata) {
                    $this->_data = UToLinkShortenerHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
