...
//    only - a typed UToLinkShortenerContext there would make every ctx
...
final class UToLinkShortenerOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new UToLinkShortenerContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new UToLinkShortenerSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new UToLinkShortenerResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new UToLinkShortenerError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new UToLinkShortenerResponse($clean['response']);
    ...
    public static function err_from_map($m): ?UToLinkShortenerError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new UToLinkShortenerError(is_string($code) ? $code : '', $msg);
    }
