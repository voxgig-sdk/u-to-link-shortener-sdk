...
// UToLinkShortener SDK feature test
//
...
class FtCtrl extends UToLinkShortenerControl
{
...
class FtClient
{
    ...
    private UToLinkShortenerUtility $utility;

    public function __construct(UToLinkShortenerUtility $utility, array $options)
    {
    ...
    public function get_utility(): UToLinkShortenerUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (UToLinkShortenerContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public UToLinkShortenerContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (UToLinkShortenerContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = UToLinkShortenerConfig::make_feature($name);
    ...
    private static function buildUrl(UToLinkShortenerSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof UToLinkShortenerError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new UToLinkShortenerResponse($fetched);
        }
    ...
    private function fail(UToLinkShortenerContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (UToLinkShortenerContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
