...
func TestLinkShorteningEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		linkShorteningRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "link_shortening"}), "link_shortening_ref01"))

		linkShorteningRef01DataResult, err := linkShorteningRef01Ent.Create(linkShorteningRef01Data, nil)
		if err != nil {
			t.Fatalf("create failed: %v", err)
		}
		linkShorteningRef01Data = core.ToMapAny(entityData(linkShorteningRef01DataResult))
...
func link_shorteningBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("U_TO_LINK_SHORTENER_TEST_LINK_SHORTENING_ENTID")
	...
	idmapResolved := core.ToMapAny(env["U_TO_LINK_SHORTENER_TEST_LINK_SHORTENING_ENTID"])
