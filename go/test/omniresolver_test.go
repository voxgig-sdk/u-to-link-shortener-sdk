...
import (
	...
	omni "github.com/voxgig-sdk/u-to-link-shortener-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.UToLinkShortenerError); is {
				out := map[string]any{
					"name":    "UToLinkShortenerError",
...
func omniCtx(arg any, client *sdk.UToLinkShortenerSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.UToLinkShortenerSDK); is {
			client = live
